...
do k = 1, size(div,3)
  ...
  if(k.le.sponge_level)then
     ...
     do n=ns,ne
        do m=ms,me
           if(m /= 0) then
              ...
              dt_div(m,n,k) = (dt_div(m,n,k) - coeff_sponge*div(m,n,k))/(1. + coeff_sponge*current_dt)
           endif
        enddo
     enddo
     ...
     if(ms == 0) then
        do n=ns,ne
           coeff_sponge = damping_zmv_sponge(n)*((sponge_level-k)/(sponge_level-1.0))**2.
           dt_div(0,n,k) = (dt_div(0,n,k) - coeff_sponge*div(0,n,k))/(1. + coeff_sponge*current_dt)
        enddo
     endif
