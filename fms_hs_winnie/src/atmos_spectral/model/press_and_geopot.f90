...
integer :: numblon,numblat,numblev            ! zw
...
! zw 
!numblon=size(surface_p,1)
!numblat=size(surface_p,2)
!my_pid=1000+mpp_pe()
!write (pid_str, "(I4)") my_pid
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/surface_p' //&
!  & pid_str // '.txt')
!write(my_pid,'(128(F20.10,2X))') surface_p
!close(my_pid)

! zw 
!open(unit=3000,file='/glade/scratch/zhengwu/FMS/model/output/sigma_levels.txt')
!write(3000,*) "numb_sigma_levels= ",size(p_half,3)," num_levels= ",num_levels
!write(3000,*) "pk= ",pk(1:num_levels)
!write(3000,*) "***************************"
!write(3000,*) "bk= ",bk(1:num_levels)
!close(3000)

! zw
!numblon=size(p_half,1)
!numblat=size(p_half,2)
!numblev=size(p_half,3)
!my_pid=1000+mpp_pe()
!write (pid_str, "(I4)") my_pid
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/p_half_37' //&
!  & pid_str // '.txt')
!write(my_pid,'(128(F20.10,2X))') p_half(:,:,37)
!close(my_pid)
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/p_half_36' //&
!  & pid_str // '.txt')
!write(my_pid,'(128(F20.10,2X))') p_half(:,:,36)
!close(my_pid)
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/p_half_35' //&
!  & pid_str // '.txt')
!write(my_pid,'(128(F20.10,2X))') p_half(:,:,35)
!close(my_pid)
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/p_half_41' //&
!  & pid_str // '.txt')
!write(my_pid,'(128(F20.10,2X))') p_half(:,:,41)
!close(my_pid)
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/p_half_nlev' //&
!  & pid_str // '.txt')
!write(my_pid,'(128(F20.10,2X))') p_half(:,:,numblev)
!close(my_pid)
