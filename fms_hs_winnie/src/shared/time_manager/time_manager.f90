...
! Given a time, computes the corresponding date given the selected calendar
 ...
 if(.not.module_is_initialized) call time_manager_init
 if(present(err_msg)) err_msg = ''
 call set_calendar_type(NOLEAP) !mj for time dependent Te and tau in hs_forcing.f90
...
! get a time_type for a year ! zw
subroutine Time_for_hs(Time2, days)
implicit none
type(time_type), intent(out) :: Time2
