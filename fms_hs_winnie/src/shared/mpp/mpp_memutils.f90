...
!mj         'Memory(MB) used in '//trim(text)//'=', mmin, mmax, mstd, mavg
         'Memory(MB) used in '//trim(text)//'=', mmin, mmax, mstd, mavg*mpp_npes()
...
!use #ifdef to generate equivalent on other platforms.
    ...
    mstd = (m-mavg)**2; call mpp_sum(mstd); mstd = sqrt( mstd/mpp_npes() )
    if( mpp_pe().EQ.mpp_root_pe() )write( mu,'(a64,4es11.3)' ) &
!mj         'Memuse(MB) at '//trim(text)//'=', mmin, mmax, mstd, mavg
         'Memory(MB) used at '//trim(text)//'=', mmin, mmax, mstd, mavg*mpp_npes()
